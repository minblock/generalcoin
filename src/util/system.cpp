...
const char * const BITCOIN_CONF_FILENAME = "generalcoin.conf";
...
#else
    const char* pszModule = "generalcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Generalcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Generalcoin";
#else
    // Unix
    return pathRet / ".generalcoin";
#endif
#endif
}
