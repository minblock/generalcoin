Generalcoin Core version 0.13.2 is now available from:

  <https://download.generalcoin.org/generalcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/generalcoin-project/generalcoin/issues>
...
updates will be released anymore. Without security updates, using a generalcoin
...
ECDSA signatures inside Generalcoin transactions now use validation using
...
Previous versions of Generalcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Generalcoin Core was to raise the default minimum
relay fee.

Generalcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Generalcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running generalcoind without having to do any manual configuration.
...
Generalcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Generalcoin Core will relay transactions with
...
entirely in Generalcoin Core 0.13, and it is at this time undecided whether the
...
Generalcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Generalcoin Core automatically creates a hidden service to listen on, without
manual configuration. Generalcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Generalcoin Core is listening, and
...
Generalcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Generalcoin
Core to use floating fees. By default, Generalcoin Core will use floating
fees.
...
(default: `0.0002` CPT/kB).

At all times, Generalcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) CPT.
Furthermore, Generalcoin Core will never create transactions paying less than
...
`generalcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /generalcoinrpc>
        ...
        # optional bypass generalcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< generalcoinrpc:<password>
    </Location>
...
Restarting generalcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in generalcoin.conf
...
generalcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/generalcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Generalcoin Core code base has
...
- `generalcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `generalcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Generalcoin scripting system that in combination with
...
Generalcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Generalcoin Core.
...
The two phases are distinct in the Generalcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Generalcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Generalcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Generalcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of generalcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of generalcoins being spent is exactly
...
- **Increased security for multisig:** Generalcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Generalcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Generalcoin's current ledger (called the UTXO set),
  ...
  note that Generalcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Generalcoin users to individually opt-in to almost any change in the Generalcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Generalcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Generalcoin
client to Generalcoin Core 0.13.2 to signal for segwit support. When a
...
Since Generalcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `generalcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `generalcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
